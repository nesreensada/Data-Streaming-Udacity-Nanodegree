...
def run_spark_streaming():
    ...
    jsonSchema = StructType([StructField('status',StringType(), True), StructField('timestamp',TimestampType(), True)])
    ...
    query = ( streamingCountsDF.writeStream.outputMode("append").format("console").start()
    )
